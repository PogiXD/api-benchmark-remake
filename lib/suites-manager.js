    ...
    addServices: function(services){
      ...
      _.forEach(this.services, function(serviceAddress, serviceName){
        _.forEach(this.routes, function(route){
          ...
          // Skip cross-protocol combinations: when an endpoint declares an
          // explicit protocol, only pair it with services whose address scheme
          // matches.  http and https are both "http-family"; grpc is distinct.
          var endpointProto = route.endpoint && route.endpoint.protocol
                              ? route.endpoint.protocol.toLowerCase()
                              : null;
          if(endpointProto) {
            var serviceScheme = /^grpc:\/\//i.test(serviceAddress) ? 'grpc' : 'http';
            var endpointFamily = endpointProto === 'grpc' ? 'grpc' : 'http';
            if(endpointFamily !== serviceScheme) {
              return;
            }
          }
          ...
          if(resolved.protocol === 'grpc'){
            ...
            var routeHref = url.resolve(resolved.address, sanitisedRoute.endpoint.route);
