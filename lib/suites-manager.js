    ...
    addServices: function(services){
      ...
      _.forEach(this.services, function(service, serviceName){
        _.forEach(this.routes, function(route){
          ...
          var protocol = route.endpoint.protocol || 'http',
              self = this,
              routeHref;

          if (protocol === 'grpc') {
            // href descrittivo per i report
            var methodPart = route.endpoint.rpcMethod || route.endpoint.method || 'Method';
            routeHref = 'grpc:///BenchmarkService/' + methodPart;

            // Inietta i parametri che grpc-request-agent.make() si aspetta.
            // L'indirizzo del servizio ha schema 'grpc://' — lo stacchiamo.
            ...
            route.endpoint.requestData  = {
              method:  rpcMethod,
              payload: Buffer.from(JSON.stringify(payload))
            };
          } else {
            routeHref = url.resolve(service, route.endpoint.route);
          }

          var routeName = serviceName + '/' + route.name;

          var agentToUse = protocol === 'grpc'
            ? self.agents.grpc
            : self.agents.http;
