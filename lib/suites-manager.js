...
var getProtocol = function(route){
  return route.endpoint.protocol || 'http';
};

var prepareGrpcRoute = function(route, service){
  var methodPart = route.endpoint.rpcMethod || route.endpoint.method || 'Method';
  var grpcAddress = service.replace(/^grpc:\/\//i, '');
  var protoFile = require('path').join(__dirname, 'grpc-benchmark.proto');
  var rpcMethod = route.endpoint.method || methodPart;
  var payload = route.endpoint.data || {};
  var requestData = route.endpoint.requestData || {
    method: rpcMethod,
    payload: Buffer.from(JSON.stringify(payload))
  };

  route.endpoint.protoFile = protoFile;
  route.endpoint.package = 'benchmark';
  route.endpoint.service = 'BenchmarkService';
  route.endpoint.address = grpcAddress;
  route.endpoint.method = 'Call';
  route.endpoint.requestData = requestData;

  return 'grpc:///Service/' + methodPart;
};

var prepareSoapRoute = function(route, service){
  route.endpoint.endpoint = service;
  route.endpoint.requestData = route.endpoint.requestData || route.endpoint.data || {};
  return service + '#' + route.endpoint.operation;
};

var prepareHttpRoute = function(route, service){
  return url.resolve(service, route.endpoint.route);
};

var getRouteHref = function(route, service){
  var protocol = getProtocol(route);

  if (protocol === 'grpc') {
    return prepareGrpcRoute(route, service);
  }

  if (protocol === 'soap') {
    return prepareSoapRoute(route, service);
  }

  return prepareHttpRoute(route, service);
};

var getAgent = function(agents, route){
  var protocol = getProtocol(route);
...
module.exports = function(agents, debugHelper){
  ...
  this.services = {};
  this.agents = {
    http: new RequestAgent(agents.http),  // superagent
    grpc: agents.grpc,                   // istanza di GrpcRequestAgent
    soap: agents.soap
  };
  ...
  return _.extend(this, {
    ...
    addServices: function(services){
      ...
      _.forEach(this.services, function(service, serviceName){
        _.forEach(this.routes, function(route){
          ...
          var self = this,
              routeHref = getRouteHref(route, service),
              routeName = serviceName + '/' + route.name,
              agentToUse = getAgent(self.agents, route);

          requestHandler.setup(routeName, routeHref, route, agentToUse);
        }, this);
      }, this);
    ...
    handleErrors: function(route){
      ...
      route.runner.on('error', function(error){
        var details = ((error && error.details) || '').substring(0, 500);
        var message = ((error && error.message) || '').substring(0, 500);
        debugHelper.log(format(settings.errorMessages.GENERIC_ERROR, details, message));
