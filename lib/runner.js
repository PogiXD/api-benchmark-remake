  ...
  this.add = function(stepName, href, stepOptions, stepRequest, func){
    ...
    step.run = function(callback){
      ...
      for(i = 0; i < cycleLength; i++) {
				self.queueInitialSample(step, sampleDone, i);
			}
    };
    this.steps.push(step);
  };

  this.queueInitialSample = function(step, sampleDone, idx){
    // setImmediate: accoda ogni runSample iniziale nell'event loop.
    // Senza questo, se gRPC risponde quasi-sincronamente, Node.js potrebbe
    // vedere l'event loop vuoto e uscire prima che la callback arrivi.
    setImmediate(function(){
      this.runSample(step, sampleDone, idx);
    }.bind(this));
  };
